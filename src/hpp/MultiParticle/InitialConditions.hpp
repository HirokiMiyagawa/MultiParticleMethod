...
#ifdef __CREASE__
    std::cout << "Analysis Crease" << endl;
    setInitialConditionsSetFlag();
    setInitialConditionsCreaseModeling();
    setInitialConditionsCrease();

#else
    std::cout << "Analysis No Crease" << endl;
...
/**
** @brief	最初の粒子間のパラメータを求める
**/
void MultiParticle::setInitialConditionsCrease(){
    for (int i = 0; i < local_iNum; i++) {
        for (int j = 0; j < local_jNum; j++) {
            for (int k = 0; k < local_kNum; k++) {
                    p->v[i][j][k].x       = 0;
